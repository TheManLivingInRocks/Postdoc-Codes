...
nx_main_region = 10000/dx + 1;     %   X方向总点数       - 2*PML_l
...
x0 = ((nx_main_region - 1)/2)*dx;    z0 = (3000/dx)*dz;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   加载参数
T_N                   = cell2mat(struct2cell(load("T_pre_invasion.mat")));
if isempty(T_N)
    error('T_N is NaN.');
end

P_hydrostatic   = cell2mat(struct2cell(load("P_pre_invasion.mat")));
if isempty(P_hydrostatic)
    error('P_hydrostatic is NaN.');
end

sigma_xx_initial = -cell2mat(struct2cell(load("sigma_H_pre_invasion.mat")));
if isempty(sigma_xx_initial)
    error('sigma_xx_initial is NaN.');
end

sigma_zz_initial = -cell2mat(struct2cell(load("sigma_v_pre_invasion.mat")));
if isempty(sigma_zz_initial)
    error('sigma_zz_initial is NaN.');
end

sigma_xx_initial(1:PML_l + 1,:) = -P_hydrostatic(1:PML_l + 1,:);
sigma_zz_initial(1:PML_l + 1,:) = -P_hydrostatic(1:PML_l + 1,:);

Rho_N = cell2mat(struct2cell(load("rho_pre_invasion.mat")));
if isempty(Rho_N)
    error('Rho_N is NaN.');
end

cV_N = Rho_N.*cell2mat(struct2cell(load("c_s_pre_invasion.mat")));
if isempty(Rho_N)
    error('cV_N is NaN.');
end

K_N = cell2mat(struct2cell(load("K_T_pre_invasion.mat")));
if isempty(Rho_N)
    error('K_N is NaN.');
end
...
if mechanism == "coupled"
    Alpha_N = cell2mat(struct2cell(load("alpha_s_pre_invasion.mat")));
elseif mechanism == "uncoupled"
    Alpha_N(1:nz_PML,1:nx_PML) = 0;
end

K_bulk_iso_N = cell2mat(struct2cell(load("K_host_dry_bulk_pre_invasion.mat")));
if isempty(K_bulk_iso_N)
    error('K_bulk_iso_N is NaN.');
end

Lambda_iso_N = cell2mat(struct2cell(load("lambda_host_pre_invasion.mat")));
if isempty(Lambda_iso_N)
    error('Lambda_iso_N is NaN.');
end

Mu_N = cell2mat(struct2cell(load("mu_host_dry_pre_invasion.mat")));
if isempty(Mu_N)
    error('Mu_N is NaN.');
end

Vs_N = sqrt(Mu_N./Rho_N);

A = isempty(Vs_N);
if isempty(Vs_N)
    error('Vs_N is NaN.');
end
Vp_iso_N = sqrt((Lambda_iso_N + 2*Mu_N)./Rho_N);
if isempty(Vp_iso_N)
    error('Vp_iso_N is NaN.');
end
Beta_N = (3*Lambda_iso_N + 2*Mu_N).*Alpha_N;
if isempty(Beta_N)
    error('Beta_N is NaN.');
...
if isempty(Young_E_N)
    error('Young_E_N is NaN.');
end
Poisson_ratio_N = Lambda_iso_N./(2.*(Lambda_iso_N + Mu_N));
if isempty(Poisson_ratio_N)
    error('Poisson_ratio_N is NaN.');
end
...
if pressure == "zero"
    P_hydrostatic(1:nz_PML,1:nx_PML)   = 0;
    sigma_xx_initial(1:nz_PML,1:nx_PML) = 0;
    sigma_yy_initial(1:nz_PML,1:nx_PML) = 0;
    sigma_zz_initial(1:nz_PML,1:nx_PML) = 0;
elseif pressure == "hydrostatic"
    P(1:nz_PML,1:nx_PML) = 50e6;
...
prestress_mode = "lithostatic";

if strcmp(prestress_mode,'hydrostatic') == 1

    exx_initial = -P./(3*K_bulk_iso_N);
    eyy_initial = -P./(3*K_bulk_iso_N);
    ezz_initial = -P./(3*K_bulk_iso_N);
    exz_initial = 0;
    ezx_initial = exz_initial;
    eyz_initial = 0;
    ezy_initial = eyz_initial;
    exy_initial = 0;
    eyx_initial = exy_initial;
    enn_initial = exx_initial + eyy_initial + ezz_initial; 

    sigma_xx_initial = C11_iso_N.*exx_initial + C12_iso_N.*eyy_initial + C13_iso_N.*ezz_initial + M11_N.*dtheta_I;
    sigma_yy_initial = C21_iso_N.*exx_initial + C22_iso_N.*eyy_initial + C23_iso_N.*ezz_initial + M22_N.*dtheta_I;
    sigma_zz_initial = C13_iso_N.*exx_initial + C23_iso_N.*eyy_initial + C33_iso_N.*ezz_initial + M33_N.*dtheta_I;

    sigma_yz_initial = C44_iso_N.*eyz_initial + C44_iso_N.*ezy_initial;
    sigma_xz_initial = C55_iso_N.*exz_initial + C55_iso_N.*ezx_initial;
    sigma_xy_initial = C66_iso_N.*exy_initial + C66_iso_N.*eyx_initial;

elseif strcmp(prestress_mode,'uniaxial') == 1
    if strcmp(direction,'X') == 1
        exx_initial = P./Young_E_N;
        eyy_initial = -Poisson_ratio_N.*P./Young_E_N;
        ezz_initial = -Poisson_ratio_N.*P./Young_E_N;
    elseif strcmp(direction,'Z') == 1
        exx_initial = -Poisson_ratio_N.*P./Young_E_N;
        eyy_initial = -Poisson_ratio_N.*P./Young_E_N;
        ezz_initial = P./Young_E_N;
    end
    exz_initial = 0;
    ezx_initial = exz_initial;
    eyz_initial = 0;
    ...
    eyx_initial = exy_initial;
    enn_initial = exx_initial + eyy_initial + ezz_initial; 
    sigma_xx_initial = C11_iso_N.*exx_initial + C12_iso_N.*eyy_initial + C13_iso_N.*ezz_initial;
    sigma_yy_initial = C21_iso_N.*exx_initial + C22_iso_N.*eyy_initial + C23_iso_N.*ezz_initial;
    sigma_zz_initial = C13_iso_N.*exx_initial + C23_iso_N.*eyy_initial + C33_iso_N.*ezz_initial;
    sigma_yz_initial = C44_iso_N.*eyz_initial + C44_iso_N.*ezy_initial;
    sigma_xz_initial = C55_iso_N.*exz_initial + C55_iso_N.*ezx_initial;
    sigma_xy_initial = C66_iso_N.*exy_initial + C66_iso_N.*eyx_initial;

elseif strcmp(prestress_mode,'pure shear') == 1
    
    exx_initial = P./(Lambda_iso_N + 2*Mu_N);
    eyy_initial = -exx_initial;
    ezz_initial = -exx_initial;
    exz_initial = 0;
    ezx_initial = exz_initial;
    eyz_initial = 0;
    ezy_initial = eyz_initial;
...
elseif strcmp(prestress_mode,'simple shear') == 1
...
elseif strcmp(prestress_mode,'lithostatic') == 1

    sigma_yy_initial = sigma_xx_initial;
    sigma_xy_initial = zeros(nz_PML,nx_PML);
    sigma_xz_initial = zeros(nz_PML,nx_PML);
    sigma_yz_initial = zeros(nz_PML,nx_PML);

    exx_initial = (1./Young_E_N).*(sigma_xx_initial - Poisson_ratio_N.*(sigma_yy_initial + sigma_zz_initial));
    eyy_initial = (1./Young_E_N).*(sigma_yy_initial - Poisson_ratio_N.*(sigma_xx_initial + sigma_zz_initial));
    ezz_initial = (1./Young_E_N).*(sigma_zz_initial - Poisson_ratio_N.*(sigma_xx_initial + sigma_yy_initial));
